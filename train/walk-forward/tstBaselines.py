...
--------------------
Author: XYZ
Description: A time series transformers based crop yield prediction framework with Walk-Forward Validation.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--early_stopping_epoch', type=int, default=None,
                        help='Maximum epoch for early stopping (E) in walk-forward mode. '
                             'If not specified, uses --epochs value. '
                             'Actual stopping epoch = min(E, train_loss_plateau_epoch).')
    parser.add_argument('--es_patience', type=int, default=5,
                        help='Early stopping patience for train loss plateau (default: 5)')
    parser.add_argument('--es_min_delta', type=float, default=1e-4,
                        help='Minimum change in train loss to qualify as improvement (default: 1e-4)')
    ...
    # Show actual epochs that will be used for walk-forward fold training
    wf_epochs = args.early_stopping_epoch if args.early_stopping_epoch is not None else args.epochs
    print(f"  lr={args.lr}  wd={args.weight_decay}  wf_epochs={wf_epochs}  "
    ...
    # Use early_stopping_epoch if specified, otherwise fall back to epochs
    wf_epochs = args.early_stopping_epoch if args.early_stopping_epoch is not None else args.epochs
    wf_results = run_walk_forward_validation(
        ...
        early_stopping_epoch=wf_epochs,
